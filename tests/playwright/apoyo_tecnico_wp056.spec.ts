...
const URL_RECINTO = 'http://localhost:3001/recinto/'
const URL_MODERACION = 'http://localhost:3000/moderacion/'
...
function transmision(parcial: Record<string, unknown> = {}) {
  return {
    estado: 'APAGADO',
    iniciada_en: null,
    en_vivo_desde: null,
...
function aviso(texto: string, destino: string) {
  return {
    aviso_id: 'aviso-e2e',
    texto,
    destino,
...
function concejalesPublicos(cantidad: number) {
...
function estadoRecinto(parcialTecnico: Record<string, unknown> = {}) {
  return {
    revision: 1,
...
function estadoTecnico(parcial: Record<string, unknown> = {}) {
  return {
    revision: 1,
    generado_en: '2026-09-02T10:00:00Z',
    estado_global: 'SESION_ABIERTA',
...
function estadoModeracion(parcialTecnico: Record<string, unknown> = {}) {
  return {
    revision: 1,
    generado_en: '2026-09-02T10:00:00Z',
    estado_global: 'SESION_ABIERTA',
